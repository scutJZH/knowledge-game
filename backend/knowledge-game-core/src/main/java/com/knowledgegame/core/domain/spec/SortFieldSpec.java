...
public final class SortFieldSpec {
     ...
     * @param allowedFields 该接口允许的字段白名单（PO 字段名 → 中文显示名）
     * @return 校验通过的 SortField（入参为 null 时返回 null，由调用方决定默认值）
     * @throws BusinessException 字段不在白名单时抛 400，消息含允许字段中文名列表
     */
    public static SortField validate(SortField sortField, Map<String, String> allowedFields) {
        if (sortField == null) {
            return null;
        }
        if (!allowedFields.containsKey(sortField.getField())) {
            throw new BusinessException(400,
                    "不支持的排序字段: " + sortField.getField()
                            + "，允许的字段: " + allowedFields.values());
        }
        return sortField;
    }
}
